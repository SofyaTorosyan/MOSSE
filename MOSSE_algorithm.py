...
def pre_process(img):
    ...
    # window has the same shape as image
...
def pre_training(args, init_frame, G):
    ...
    # element wise multiplication but in fourier domain
    Ai = G * np.conjugate(np.fft.fft2(fi))
    Bi = np.fft.fft2(init_frame) * np.conjugate(np.fft.fft2(init_frame))

    # H = Ai / Bi is complex conjugate of the filter
    
    # pretrain on first frame
...
def MOSSE_tracking(args, img_path):
    ...
    init_img   = cv2.imread(img_list[0]) # get the image of first frame
    init_frame = cv2.cvtColor(init_img, cv2.COLOR_BGR2GRAY) # get gray scale image of the initial image
    init_frame = init_frame.astype(np.float32)  # matrix (360, 480)

    # get the init ground truth [x, y, width, height]
    # coordinates fro selected box: upper left coordinates, width, height
    fromCenter = False
    ...
    for idx in range(len(img_list)):
        ...
        current_frame = cv2.imread(img_list[idx])
        frame_gray = cv2.cvtColor(current_frame, cv2.COLOR_BGR2GRAY)
        frame_gray = frame_gray.astype(np.float32)
        if idx == 0: # if this is the first frame
            Ai = args.lr * Ai
            Bi = args.lr * Bi
            pos = init_gt.copy() # keep previous position
            clip_pos = np.array([pos[0], pos[1], pos[0]+pos[2], pos[1]+pos[3]]).astype(np.int64) 
        else: # else if this isn't the first frame
